...
priority_file <- '/global/homes/g/grabowsp/data/switchgrass/reseq_metadata/8X_Priority_List_FV.csv'

priority_df <- read.table(priority_file, sep = ',', header = T, 
  stringsAsFactors = F)
...
priority_df$pick_v2 <- priority_df$Pick.Sequence

priority_df$ACC <- NA
priority_df$STATE <- NA
priority_df$COLLECTION_TYPE <- NA

for(i in seq(nrow(priority_df))){
  tmp_ind <- which(meta$PLANT_ID == priority_df$PLANT_ID[i])
  priority_df$ACC[i] <- meta$ACC[tmp_ind]
  priority_df$STATE[i] <- meta$STATE[tmp_ind]
  priority_df$COLLECTION_TYPE[i] <- meta$COLLECTION_TYPE[tmp_ind]
}

south_inds <- grep('Florida|Georgia|Louisiana|Mississippi|Texas',
  priority_df$STATE)
priority_df$pick_v2[south_inds] <- 'Yes'

east_inds <- c(grep('Maine|Maryland|Massachusetts|New Jersey|North Carolina', 
  priority_df$STATE), which(priority_df$STATE == 'Virginia'), 
  intersect(which(priority_df$STATE == 'New York'), 
    which(priority_df$LATITUDE < 41.53))) 


# I want at least one J532 sample (J532.A, J532.C)
#   both have non-replicated phenotypes
#   Will just go with J532.A
# I want the J616 sample
# I want J195 sample
# Do NOT need J668 - already have sample from that accession
# Do NOT need J620.A

# only 3 of the "east_inds" that aren't already in the list are ones
#   I want to keep
east_inds_2 <- c(which(priority_df$PLANT_ID == 'J532.A'),
  which(priority_df$PLANT_ID == 'J616.A'),
  which(priority_df$PLANT_ID == 'J195.A'))

priority_df$pick_v2[east_inds_2] <- 'Yes'

# 1 sample from Arizona accession
priority_df$pick_v2[which(priority_df$PLANT_ID == 'J623.A')] <- 'Yes'
# sample from Tennessee
priority_df$pick_v2[which(priority_df$PLANT_ID == 'J342.C')] <- 'Yes'


# get accessions that have not been sequenced and are not represented
#  in the "yes" samples

potential_accs <- unique(priority_df$ACC[which(priority_df$pick_v2 == 'TBA')])

pa_notseq <- c()

for(j in potential_accs){
  tmp_meta_inds <- which(meta$ACC == j)
  if(sum(is.na(meta$LIBRARY[tmp_meta_inds])) == length(tmp_meta_inds)){
    pa_notseq <- c(pa_notseq, j)
  }
}

pa_notyet <- c()
for(k in pa_notseq){
  tmp_inds <- which(priority_df$ACC == k)
  if('Yes' %in% priority_df$pick_v2[tmp_inds] == F){
    pa_notyet <- c(pa_notyet, k)
  }
}

# this includes:
J298: NAM line - take out
J025.A = Cave-in-Rock - keep
J443.B = Pathfinder - keep
J038.A = Trailblazer - keep
J162.A = Blackwell - keep
J285.A = Hammonasset - keep
J289.A = Blackwell - keep

pa_notyet <- setdiff(pa_notyet, 'J298')

not_represented_yet <- c()

for(m in pa_notyet){
  tmp_inds <- which(priority_df$ACC == m)
  not_represented_yet <- c(not_represented_yet, tmp_inds[1])
}

# don't really need these, except for the cultivars

cultivar_keeps <- intersect(not_represented_yet, 
  which(priority_df$COLLECTION_TYPE == 'Cultivar'))

priority_df$pick_v2[cultivar_keeps] <- 'Yes'

# none of the additional mixed-ploidy samples are at Pickle

priority_df_keep <- priority_df[, c(1:6)]

out_file  <- '/global/homes/g/grabowsp/data/switchgrass/reseq_metadata/8X_Priority_List_FV_v2.txt'

write.table(priority_df_keep, file = out_file, quote = F, sep = '\t', 
  row.names = F, col.names = T)

topoff_df <- ploidy[which(ploidy$seq_cov < 1e10), c('PLANT_ID', 'lib')]
colnames(topoff_df) <- c('PLANT_ID', 'LIBRARY')

topoff_file <- '/global/homes/g/grabowsp/data/switchgrass/reseq_metadata/8X_Libraries_for_increased_coverage.txt'

write.table(topoff_df, file = topoff_file, quote = F, sep = '\t', 
  row.names = F, col.names = T)
...
meta[grep('Hammonasset', paste(meta$SOURCE_ID_1, meta$SOURCE_ID_2)),
  c('ACC', 'PLANT_ID', 'LIBRARY', 'PLOIDY', 'COLLECTION_TYPE')]


meta[which(meta$STATE == 'Arizona'),
c('ACC', 'PLANT_ID', 'LIBRARY', 'PLOIDY', 'COLLECTION_TYPE', 'LATITUDE',
  'LONGITUDE')]



meta[which(meta$ACC == 'J402'),
  c('ACC', 'PLANT_ID', 'LIBRARY', 'PLOIDY', 'COLLECTION_TYPE', 'LATITUDE',
  'LONGITUDE')]

phenos[grep('J623', phenos$PLANT_ID), ]
