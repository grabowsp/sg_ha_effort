...
### LOAD DATA ###
#data_file <- args[1]
data_file <- paste('/global/cscratch1/sd/grabowsp/', 
  'sg_ploidy/polyploid_genos_popstructure/expand_geo839_dists/', 
  'Chr01K.polyploid.CDS.expandgeosamps.ploidy_DistMat.total.rds', sep = '')
...
out_file_subpop <- paste(out_file_pre, out_file_dist_suf, 
  'subpop_colors.large_cladogram.NJ_tree.pdf', sep = '.')
out_file_ploidy <-  paste(out_file_pre, out_file_dist_suf, 
  'ploidy_colors.large_cladogram.NJ_tree.pdf', sep = '.')
...
pdf(file = out_file_subpop, width = 20, height = 70)
plot(dist_nj, type = 'phylogram', use.edge.length = F, no.margin = T,
