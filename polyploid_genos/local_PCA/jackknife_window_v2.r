# Script to to use jackknife approach to test signal vs noise for SNP- and
#  bp-window combination 
...
snp_win_size <- as.numeric(args[7])
#snp_win_size <- 500

bp_win_size <- as.numeric(args[8])
# bp_win_size <- 3e4

# the number of PC's to retain and test
test_k <- as.numeric(args[9])
# test_k <- 2

# vector to use for eventually test each PC up to test_k
pcs_to_check <- seq(test_k)
...
for(pct in pcs_to_check){
  pc_test_pre <- paste('PC_', pct, '_', sep = '')
  pc_tot_vec_cols <- grep(pc_test_pre, colnames(tot_ew))
  #
  tot_pc_mat <- tot_ew[, pc_tot_vec_cols]
  ...
  res_mat$tot_mean_variance[which(res_mat$PC == pct)] <- tot_mean_variance
}

# Calculate variance-noise from jackknife eigen-windows
