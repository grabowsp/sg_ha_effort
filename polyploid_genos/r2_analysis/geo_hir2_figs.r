...
combo_short <- 'combined.geo_subgroup.hi_r2.10bpwindow.rds'
combo_file <- paste(res_dir, combo_short, sep = '')

combo_r2 <- readRDS(combo_file)

chrom_short <- 'Chromosome.geo_subgroup.hi_r2.10bpwindow.rds'
...
for(i in names(combo_r2)){
  colnames(combo_r2[[i]])[2] <- paste(i, '_per_high_ld', sep = '')
}
...
for(i in seq(length(combo_r2))){
  ...
  ylab('% r^2 above 0.9') +
  ylim(0,0.4) +
   ggtitle(paste('% r^2 above 0.9 in ', names(combo_r2)[i],
    '\ngenome-wide 10bp windows'))
}

combo_fig_file <- paste(res_dir, 'combo_geo_subgroup_hi.r2_fig_1.pdf',
  sep = '')
...
for(i in seq(length(combo_r2))){
  wind_r2_df[, names(combo_r2)[i]] <- combo_r2[[i]][,2]
}
...
gg_tot_groups <- ggplot(wind_melt, aes(x = window_pos, y = value)) +
  ...
  xlab('distance between SNPs') +
  ylab('% r^2 above 0.9') +
  ggtitle('% r^2 above 0.9 in 10bp windows in geo_samps subgroups')
  
combo_tot_file <- paste(res_dir, 'combo_geo_subgroup_total_hi.r2_fig_1.pdf',
  sep = '')
...
chr_uni_df <- chrom_unified[[1]][, c(1,5,2)]
for(cn in c(2:length(chrom_names))){
  chr_uni_df[, paste(chrom_names[cn], '_per_high_ld', sep = '')] <- 
...
for(i in seq(length(chrom_names))){
  ...
  xlab('distance between SNPs') +
  ylab('% r^2 above 0.9') + 
  ggtitle(paste('% r^2 above 0.9 in 10bp windows in ', chrom_names[i], 
    '\ngeo_samps subgroups'))
#  print(chr_plot_ls[[i]])
}

chr_plot_out <- paste(res_dir, 'chromosome_geo_subgroup_hi.r2_fig_1.pdf', 
  sep = '')
