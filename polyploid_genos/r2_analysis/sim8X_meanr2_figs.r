...
sim_combo_short <- 'combined.sim8X_subgroups.mean_r2.10bpwindow.rds'
...
geo_combo_short <- 'combined.geo_subgroup.mean_r2.10bpwindow.rds'
...
for(i in names(sim_combo_r2)){
  colnames(sim_combo_r2[[i]])[2] <- paste(i, '_window_avg', sep = '')
}
...
for(i in seq(length(sim_combo_r2))){
  ...
  ylab('mean r^2') +
  ylim(0,0.5) +
   ggtitle(paste('r^2 in ', names(sim_combo_r2)[i],
    '\ngenome-wide 10bp windows'))
}

combo_fig_file <- paste(sim_res_dir, 'combo_sim8X_subgroup_mean.r2_fig_1.pdf',
  sep = '')
...
for(i in seq(length(sim_combo_r2))){
  wind_r2_df[, names(sim_combo_r2)[i]] <- sim_combo_r2[[i]]$window_avg
}
...
gg_tot_groups <- ggplot(wind_melt, aes(x = window_pos, y = value)) +
  ...
  xlab('distance between SNPs') +
  ylab('mean r^2') +
  ggtitle('Mean r^2 in 10bp windows in sim8X subgroups')
  
combo_tot_file <- paste(sim_res_dir, 
  'combo_sim8X_subgroup_total_mean.r2_fig_1.pdf', sep = '')
...
for(tg in geo_group_names){
  ...
  wind_r2_df <- data.frame(window_pos = sim_combo_r2[[1]]$window_pos,
    stringsAsFactors = F)
  wind_r2_df[, tg] <- geo_combo_r2[[tg]]$window_avg

  for(i in oct_groups){
    wind_r2_df[, i] <- sim_combo_r2[[i]]$window_avg
  }
  wind_melt <- melt(wind_r2_df,
    measure.vars = colnames(wind_r2_df)[c(2:ncol(wind_r2_df))])
  sim_tet_gg_list[[tg]] <- ggplot(wind_melt, aes_string(x = 'window_pos',
    ...
    ylab('mean r^2') +
    ylim(0.05, 0.6) +
    ggtitle('Mean r^2 in 10bp windows in 4X and associated sim8X subgroups')
}

sim_tet_file <- paste(sim_res_dir,
  'combo_4X_subgroup_withsim8X_subgroup_total_mean.r2_fig_1.pdf', sep = '')
