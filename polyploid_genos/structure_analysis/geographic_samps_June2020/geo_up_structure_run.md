...
for KT in {1..10};
  ...
  for KR in {1..3};
    do
    grep 'Estimated Ln Prob of Data' upgeo_$KT'.'$KR'_f' >> \
      upgeo_LnProbData.txt;
    done;
  done;
```
* in R
...
ln_prob_in <- '/home/t4c1/WORK/grabowsk/data/switchgrass/polyploid_genos/struc/up_geo/upgeo_LnProbData.txt'
...
gg_lnprob <- ggplot(delta_df, aes(x = K, y = mean_ln_prob)) +
  ...
  xlab('K') +
  ylab('ln Prob Data') +
  ggtitle('Ln Prob Data for Upland Geographic Samples')
...
gg_change1 <- ggplot(delta_df, aes(x = K, y = mean_change1_K)) +
  ...
  xlab('K') +
  ylab("L'(K)") +
  ggtitle("L'(K) for Upland Geographic Samples")
...
gg_deltaK <- ggplot(delta_df, aes(x = K, y = delta_K)) +
  ...
  xlab('K') +
  ylab("delta K") +
  ggtitle("delta K for Upland Geographic Samples")

out_fig_file <- '/home/t4c1/WORK/grabowsk/data/switchgrass/polyploid_genos/struc/up_geo/up_geo_structure_evanno.pdf'

pdf(out_fig_file, width = 12, height = 10)
(gg_lnprob + gg_change1) / (gg_change2 + gg_deltaK)
dev.off()

out_tab_file <- '/home/t4c1/WORK/grabowsk/data/switchgrass/polyploid_genos/struc/up_geo/up_geo_structure_lnprob.txt'
