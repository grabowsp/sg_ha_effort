...
group_col_list <- list()
group_col_list[[1]] <- c('red2', 'yellow3')
group_col_list[[2]] <- c('red2', 'yellow3', 'salmon2')
group_col_list[[3]] <- c('orange2', 'red2', 'salmon2', 'yellow3')
names(group_col_list) <- as.character(k_vec)
# * input the group colors, as determined previously

group_name_list <- list()
group_name_list[[1]] <- c('Texas', 'East Coast')
group_name_list[[2]] <- c('Texas', 'East Coast', 'Gulf Coast')
group_name_list[[3]] <- c('Northern\nEast Coast', 'Texas', 'Gulf Coast', 
  'Southern\nEast Coast')
...
pop_order_list[[2]] <- c(1,3,2)
pop_order_list[[3]] <- c(2,3,4,1)
