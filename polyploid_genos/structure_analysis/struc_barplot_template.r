...
# look at the info about each population
pop_info_list
...
# set the order for the populations in the plot
pop_order <- c(2,3,1,4)

group_col_vec <- c('darkorchid2','blue2','steelblue2','turquoise4')
names(group_col_vec) <- seq(length(group_col_vec))

group_lab_vec <- c('Mainly 8X - Cosmo','4X Dakotas','4X Great Lakes',
  '8X MidEast')
names(group_lab_vec) <- seq(length(group_lab_vec))
