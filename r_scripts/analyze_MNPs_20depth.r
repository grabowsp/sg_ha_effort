...
count_dir <- '/global/cscratch1/sd/grabowsp/sg_ploidy/MNP_counts/depth20/'
...
### SET OUTPUTS ###
fig_out_dir <- '/global/cscratch1/sd/grabowsp/sg_ploidy/MNP_counts/depth20/'

fig_prefix <- 'mnp_hist_20depth_'

title_prefix <- 'MNP with 20+ depth per allele'
### SET VARIABLES ###
...
for(LIB in tmp_libs){
  tmp_count_file <- paste(count_dir, LIB, '_20_MNP_total_count.txt', sep = '')
